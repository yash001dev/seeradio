...
import "./CampaignDetail.styles.css";
import Dropzone from "../../components/UI/Dropzone/dropzone";

import { useParams } from "react-router";
import { Table } from "reactstrap";
import Layout from "./../../components/Layout/Layout.index";
import { getCampaignByID } from "../../constant/api";
...
function CampaignDetail(props) {
  ...
  useEffect(() => {
    setIsLoading(true);
    getCampaignByID(id, props.userId)
      .then((res) => {
        setIsLoading(false);
        setCampaignData({
          ...
          actionRequiredBy:
            res.data.statusWithPerson.firstName +
            " " +
            res.data.statusWithPerson.lastName,
        });
        let tempAdvertiserAssets, tempScriptFile, tempVoiceFile;
        tempAdvertiserAssets = res.data.CampaignAssets.filter(
          (file) => file.type === "OTHER"
        );
        tempScriptFile = res.data.CampaignAssets.filter(
          (file) => file.type === "SCRIPT"
        );
        tempVoiceFile = res.data.CampaignAssets.filter(
          (file) => file.type === "AUDIO"
        );

        setAssets({
          advertiserAssets: tempAdvertiserAssets,
          scriptFile: tempScriptFile,
          voiceFile: tempVoiceFile,
        });
      })
      .catch((err) => {
        setIsLoading(false);
          ...
          <div className="campaignDetailPage">
            <div className="top-section column p-4">
              <Row>
                <Col>
                  <h6>Advertiser</h6>
                  <p>{campaignData.advertiser}</p>
                </Col>
                <Col>
                  <h6>Order Name</h6>
                  <p>{campaignData.orderName}</p>
                </Col>
                <Col>
                  <h6>Order Number</h6>
                  <p>{campaignData.orderNumber}</p>
                </Col>
                <Col>
                  <h6>Sales Organization</h6>
                  <p>{campaignData.salesOrganization}</p>
                </Col>
              </Row>
            </div>
            <div className="middle-section column px-5 py-3">
              <Row>
                ...
                <Col>
                  ...
                  <p>{campaignData.actionRequiredBy}</p>
                </Col>
                <Col>
            ...
            <div className="bottom-section column px-5 py-3">
              ...
              <Row className="mb-3">
                <Col md={4}>
                  <h6>Account Manager Assigned</h6>
                  <p>text</p>
                </Col>
                <Col md={4}>
                  <h6>Order Name</h6>
              ...
              <Row className="m-3">
                <Col> Advertiser Assets Required</Col>
              </Row>
              <Row>
                <i
                  className="fa fa-download"
                  style={{ color: "#01a1dd", fontWeight: "bold" }}
                  aria-hidden="true"
                >
                  {" "}
                  Download all assets
              ...
              <div className="advertiserAssetsTable">
                <Table className="striped">
                  ...
                  <tbody>
                    {assets.advertiserAssets !== null &&
                      assets.advertiserAssets.map((file) => {
                        var monthNames = [
                          "Jan",
                          "Feb",
                          "Mar",
                        ...
                        return (
                          <tr key={file.id}>
                            ...
                            <td md={3}>
                              {file.uploadedByPerson.firstName +
                                " " +
                                file.uploadedByPerson.lastName}
                            </td>
                            <td md={3}>
                              {fromDate.getDate() +
                                "-" +
              ...
              <Row>
                <Col className="mb-3" md={4}>
                  <h6>Preffered landing website url</h6>
                  <p>{campaignData.landingWebsiteURL}</p>
                </Col>
                <Col className="mb-3" md={4}>
                  <h6>Distribution Budget</h6>
...
const mapStateToProps = (state) => ({
  userId: state.person.person.token,
});
